...
class Errors:
    def __init__(self, y_test, y_hat, config, run_id=None, c_id="C-1"):
        """
        ...
        Attributes:
            ...
            normalized (arr): (deprec) prediction errors as a percentage of the range
        """

        self.config = config
        self.window_size = self.config['window_size']
        self.n_windows = int(
            (y_test.shape[0] - (self.config['batch_size'] * self.window_size))
        ...
        self.e = [abs(y_h - y_t[0]) for y_h, y_t in zip(y_hat, y_test)]
        ...
        if config['smoothing_perc'] is not None:
            ...
            self.e_s = np.array(self.e).flatten()

        # for values at beginning < sequence length, just use avg
        if not c_id == "C-2":  # anomaly occurs early in window
        ...
        # self.normalized = np.mean(self.e / np.ptp(y_test))
        # print("normalized prediction error: {0:.2f}".format(self.normalized))
        ...
        while self.n_windows < 0:
            ...
            if self.window_size == 1 and self.n_windows < 0:
                raise ValueError(
                    ...
                    "Adjust in config".format(
                        self.config['batch_size'], y_test.shape[0]
                    )
                )
    ...
    def process_batches(self, y_test):
        ...
        for i in range(0, self.n_windows + 1):
            ...
            window = ErrorWindow(y_test, self.config, prior_idx, idx, self.e_s, i)
            # print(window.e_s.shape)
        ...
        if len(self.i_anom) > 0:
            ...
            groups = [list(group) for group in consecutive_groups(self.i_anom)]
...
class ErrorWindow:
    def __init__(self, y_test, config, start_idx, end_idx, e_s, window_num):
        """
        ...
        Args:
            ...
            end_idx (int): Ending index for window within full set of channel
                test values
            e_s (arr): smoothing errors abs(y_hat - y_test)
    ...
    def find_epsilon(self, inverse=False):
        ...
        for z in np.arange(2.5, self.sd_lim, 0.5):
            ...
            if len(i_anom) > 0:
                # group anomalous indices into continuous sequences
                groups = [list(group) for group in consecutive_groups(i_anom)]
